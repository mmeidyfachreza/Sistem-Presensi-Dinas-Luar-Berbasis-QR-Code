                ...
                <table class="table table-hover text-nowrap">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$assignmentOrder->start_date}}</td>
                                ...
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    @can('assignment order show')
                                        <a class="btn btn-info" href="{{route('assignment_order.show',$assignmentOrder->id)}}"><i
                                            class="fa fa-eye"></i></a>
                                    @endcan
                                    @can('assignment order edit')
                                        <a class="btn btn-warning" href="{{route('assignment_order.edit',$assignmentOrder->id)}}" data-toggle="tooltip" title="Ubah Data"><i
                                            class="fa fa-pen"></i></a>
                                    @endcan
                                    @can('assignment order delete')
                                        <form class="btn-group delete-form" role="group" action="{{route('assignment_order.destroy',$assignmentOrder->id)}}" method="POST">
                                        {{ csrf_field() }}
                                        @method('DELETE')
                                        <button class="btn btn-danger delete-confirms" value="{{$assignmentOrder->name}}" type="submit" data-toggle="tooltip" title="Hapus Data"><i
                                            class="fa fa-trash"></i></button>
                                    </form>
                                    @endcan
                                    @can('assignment order print')
                                    @if ($assignmentOrder->status == "disetujui")
                                    <form action="{{route('assignment_order.print',$assignmentOrder->id)}}" method="GET" class="btn-group" role="group" target="__blank">
                                        <button class="btn btn-primary" type="submit" data-toggle="tooltip" title="Cetak Surat"><i
